...
function munge_method_name(protocol, method_name, arity) {
    return munge(protocol.fullname + "--" + method_name + "-" + arity)
}

function define_protocol(mod, name, signatures) {
    let proto = new Protocol(mod.pkg + ":" + mod.name + "/" + name, signatures)
...
// piglet.lang and thus creating circular dependencies
...
function satisfied(fullname, obj) {
    return registry[fullname].satisfied(obj)
}

export default Protocol
export {define_protocol, extend_protocol, extend_class, munge_method_name, invoke_proto, satisfied}
