...
class FnTail extends ASTNode {
  constructor(form, name, argv, body, arities) {
    super(form)
    this.argv = argv
    this.body = body
...
class FnExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    let name, args_form, argv, body = [], metadata
    this.form = form
    let [_, x, ...rest] = form
    ...
    try {
      ...
      const varargs_p = args_form.find((a)=>is_sym_name(a, "&"))

      ////// Args
      // No destructuring or varags? Emit plain function arguments
      if (args_form.every(symbol_p) && !varargs_p) {
      ...
      metadata = metadata.assoc(arities_kw, varargs_p ? [[argv.findIndex((a)=>is_sym_name(a, "&")), more_kw]] : [count(argv)])
