...
class DefExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        console.log("DefExpr form:")
        console.log(form)
        const [_def, var_sym, docStringOrValue, value] = form
        //current_module().intern(var_sym.name, null, meta(var_sym))
        if (docStringOrValue && value && typeof docStringOrValue === "string") {
            // ignore the doc string
            return new this(form, var_sym, analyzer.analyze(value))
        } else if (docStringOrValue && value === undefined) {
            // doc string does not exists 
            return new this(form, var_sym, analyzer.analyze(docStringOrValue))
        } else {
            throw new Error("Invalid arguments to def")
        }
    }
    emit(cg) {
