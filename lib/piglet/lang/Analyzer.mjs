...
class ClassMethod extends ASTNode {
    static valid_identifier = /^[_$a-zA-Z\xA0-\uFFFF][_$a-zA-Z0-9\xA0-\uFFFF]*$/
    static reserved_keyword = /^(do|if|in|for|let|new|try|var|case|else|enum|eval|false|null|this|true|void|with|break|catch|class|const|super|throw|while|yield|delete|export|import|public|return|static|switch|typeof|default|extends|finally|package|private|continue|debugger|function|arguments|interface|protected|implements|instanceof)$/
    ...
    static from(form, kind, analyzer) {
        ...
        try {
            if (seq_p(fname) && is_sym_name(first(fname), "unquote")) {
                return new this(
                    form,
                    kind,
                    analyzer.analyze(second(fname)),
                    this.analyze_fntail(form, fntail, analyzer),
                    true,
                    !!get(meta(fname), static_kw)
                )
            }
            let is_valid = this.valid_identifier.test(fname.name) && !this.reserved_keyword.test(fname.name)
            return new this(
                form,
                kind,
                is_valid ? JsIdentifierExpr.from(fname) : ConstantExpr.from(fname.name),
                // HACK: analyzed_form is needed here to unwrap the MetaExpr so we
                // get a plain FnExpr. We can't wrap a class method in a call to
                // set_meta. We do need to propagate the form meta for location
                // metadata.
...
class ClassField extends ASTNode {
    ...
    static from(form, analyzer) {
        if (seq_p(form) && is_sym_name(first(form), "unquote")) {
            return new this(
                form,
                analyzer.analyze(second(form)),
                undefined,
                (get(meta(form), static_kw)),
                true)
        }
        let [fname, fval] = seq_p(form) ? form : [form, undefined]
        let computed = seq_p(fname) && is_sym_name(first(fname), "unquote")
        return new this(
            form,
            computed ? analyzer.analyze(second(fname)) : JsIdentifierExpr.from(fname),
...
class ClassExpr extends ASTNode {
    ...
    emit(cg) {
        return cg.class_expr(
            ...
            this.fields.concat(this.methods).map((f)=>cg.emit(f, f)),
        )
    }
}
