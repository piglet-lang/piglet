...
const first_qsym = qsym('https://piglet-lang.org/packages/piglet:lang:first')
const rest_qsym = qsym('https://piglet-lang.org/packages/piglet:lang:rest')
...
class AssignmentBase extends ASTNode {
    constructor(form, var_expr, value) {
        super(form)
    ...
    emit(cg) {
        return cg[this.constructor.code_gen_method](this, cg.emit(this, this.var_expr), cg.emit(this, this.value))
    }
}
...
class BindingPair extends ASTNode {
    static from(pair, analyzer, Assignment) {
        ...
        if (!PairType)
            throw new Error("Left-hand side of binding pair must be symbol, vector, or dict")
...
class VectorBinding extends BindingPair {
    ...
    static from(pair, lhs_orig, rhs, analyzer, Assignment) {
        const self         = new this(pair)
        const carrier_sym  = new gensym("seq-carrier")
...
class DictBinding extends BindingPair {
    static from(pair, lhs, rhs, analyzer, Assignment) {return new this(lhs, rhs)}
}
        ...
        return cg.define_var(
            ...
            cg.function_expr(
                ...
                {name: this.name,
                 argv: this.argv.map(e=>e.emit(cg)),
                 body: this.body.map(e=>e.emit(cg))}),
            cg.emit(this, Associative._assoc(meta(this.var_sym), keyword("macro"), true)))
    }
}
