...
class ModuleExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        interned_mod.merge_opts(mod_opts)
        interned_mod.self_ref = gensym(`mod-${interned_mod.name}`)
        ...
        const children = [
            ...
            analyze(list(symbol('.merge_opts'), self, list(symbol('quote'), mod_opts))),
            analyze(list(symbol('set!'), list(symbol(".-location"), self), compiler().loading_path)),
