...
class FnTail extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    const bindings = flatten_assignment_pair(args_form, symbol("js:arguments"))
    return new this(form, [], [BindingScope.from(form, bindings, (a)=>a.analyze_forms(rest), ConstAssignment, analyzer)], arity, varargs)
...
function flatten_assignment_pair(lhs, rhs) {
  ...
  if (vector_p(lhs)) {
    ...
    let carrier = with_meta(gensym("car"), dict("carrier", true)) // mark the carrier so we don't introduce a var
...
class RecurExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    return analyzer.with_loop_tail((binding_forms)=>{
      ...
      return new this(
        form,
        [...bindings.map((pair)=>new Reassignment(pair, analyzer.analyze_without_meta(pair[0]), pair[1])),
         ContinueStmt]
      )
    })
  }
...
export default class Analyzer {
  ...
  /**
   * Temporarily set a specific locals stack, use with capture_locals
   */
  with_locals_stack(stack, thunk) {
    const orig_stack = this.locals_stack
    this.locals_stack = stack
    try {
      return thunk()
    } finally {
      this.locals_stack = stack
