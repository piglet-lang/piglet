...
class FnExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        analyzer.push_locals_no_munge(name ? [name] : [])
...
class ModuleExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        const current_package = compiler().pkg
        const mod_opts = Module.parse_opts(current_package, form)
        const parsed_mod = new Module(mod_opts)
        ...
        const children = [
            ...
            analyze(list(symbol('.set_opts'), self, list(symbol('quote'), mod_opts))),
            ...
            ...(interned_mod.imports.map(({from, alias})=>{
                if (from instanceof PrefixName) {
                    return analyze(list(symbol('await'), list(symbol('piglet:lang:require'), list(symbol('piglet:lang:prefix-name'), from.prefix, from.suffix))))
...
class TypeofExpr extends ASTNode {
    ...
    emit(cg) {
        return cg.typeof_expression(this, cg.emit(this.argument, this.argument))
    }
}
...
export default class Analyzer {
    ...
    analyze(form) {
        if (seq_p(form)) {
            ...
            if (initial instanceof Sym) {
                ...
                if (get(meta(the_var), keyword("macro"))) {
                    ...
                    return (this.analyze(the_var(...rest(form))))
                }
                if (the_var) {
        ...
        if (form instanceof PrefixName) {
            return new ConstantExpr(current_context().expand(form))
        }
    ...
    push_locals_no_munge(list) {
        this.locals_stack.unshift(Array.from(list).reduce((acc, s) => (acc[s.name]=s.name, acc), {}))
    }
