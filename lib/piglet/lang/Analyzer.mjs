...
class ClassExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        let f = more.shift()
        while (seq_p(f)) {
            if (is_sym_name(first(f), "extends")) {
                self.superclass = analyzer.analyze(second(f))
            } else if (is_sym_name(first(f), "constructor")) {
                analyzer.push_locals_no_gensym([super_sym, this_sym])
                self.methods.unshift(
                    {kind: 'constructor',
                     key: JsIdentifierExpr.from(first(f)),
                     // HACK: analyzed_form unwraps the MetaExpr so we get a plain FnExpr
                     value: analyzer.analyze(with_meta(list_STAR(fn_sym, rest(f)), meta(f))).analyzed_form})
                analyzer.pop_locals()
            } else if (is_sym_name(first(f), "field")) {
                self.fields.unshift(
                    {key: JsIdentifierExpr.from(second(f)),
                     value: count(f)>2 ? analyzer.analyze(third(f)) : undefined,
                     is_static: (get(meta(first(f)), static_kw) ||
                                 get(meta(second(f)), static_kw))})
            }
            f = more.shift()
        }
        if (self.identifier) {
    ...
    emit(cg) {
        let methods = this.methods.map(
            ({kind, key, value, computed, is_static}) => cg.class_method(kind, key.emit(cg), value.emit(cg), computed, is_static)
        )
        let fields = this.fields.map(
            ({key, value, computed, is_static}) => cg.class_field(key.emit(cg), value === undefined ? undefined : value.emit(cg), computed, is_static)
        )
