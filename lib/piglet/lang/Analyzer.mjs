...
class ArityDispatch extends ASTNode {
  ...
  static from(form, analyzer) {
    let body = []
    try {
      const arity_assignment = ConstAssignment.from(null, analyzer, symbol("arity"), symbol("js:arguments.length"))
      body.push(arity_assignment)
      const arity_sym = arity_assignment.lhs_expr
      // Each FnTail only has a single arity, sort by ascending arity. Varargs
      // come last.
      const sort_val = (tail) => tail.varargs ? 1000 + tail.arity : tail.arity
      let tails = Array.from(form, (tail_form)=>FnTail.from(tail_form, analyzer)).sort((a, b)=>sort_val(a)-sort_val(b))
      body.push(
        reduce(
          (acc, tail) => new IfStmt(
            tail.form,
            new InfixOpExpr(null, tail.varargs ? "<=" : "===", [tail.arity, arity_sym]),
            new DoExpr(
              tail.form,
...
class StaticBlock extends ASTNode {
  ...
  static from(form, analyzer) {
    try {
      analyzer.push_locals_no_gensym([this_sym])
      return new this(form, analyzer.analyze(form))
    } finally {
      analyzer.pop_locals()
    }
