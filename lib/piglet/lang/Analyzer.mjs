...
class ModuleExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        interned_mod.location = compiler().loading_path
        ...
        const children = [
                    ...
                    list(symbol('.register_module'),
                         ...
                         list(symbol('quote'),
                              {pkg: interned_mod.pkg,
                               ...
                               context: interned_mod.context.entries,
            ...
            //     ))),
            // analyze(list(symbol('.merge_opts'), self, list(symbol('quote'), {"imports": }))),
            ...
            ...(interned_mod.imports.map(({from, alias, js_module, module_path})=>{
                if (js_module) {
                    return analyze(list(symbol('.set_alias'), self, alias.name, list(symbol('await'), list(symbol('piglet:lang:js-import'), module_path))))
                }
                return analyze(list(symbol('await'), list(symbol('piglet:lang:require'), from.fqn)))
            })),
            self
        ]
