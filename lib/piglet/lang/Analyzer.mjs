...
class FnExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        try {
            if (argv.every(symbol_p) && !(argv.find((a)=>is_sym_name(a, "&")))) {
                for (const a of argv)
                    return new this(form, name,
                                    ...
                                    argv.map(a=>(analyzer.push_locals([a]), analyzer.analyze(a))),
                                    rest.map(f=>analyzer.analyze(f)),
                                    metadata)
            }
            body = seq(argv) ? [BindingPair.from([argv, symbol("js:arguments")], analyzer, ConstAssignment)] : []
            body = body.concat(rest.map(f=>analyzer.analyze(f)))
            return new this(form, name, [], body, metadata)
        } finally {
...
class LetExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        children.push(...body.map(e=>analyzer.analyze(e)))
        analyzer.reset_locals(locals_stack)
...
class LoopExpr extends ASTNode {
    ...
    static analyze_tail_form(form, analyzer) {
        ...
        if (seq_p(form)) {
            ...
            if (initial instanceof Sym && initial.mod == null) {
                ...
                case "do":
                    return new DoExpr(form,
                                      analyzer
                                      .analyze_forms(butlast(rest(form)))
                                      .concat([this.analyze_tail_form(last(form), analyzer)]))
                default:
                }
            }
        }
    ...
    static from(form, analyzer) {
        ...
        self.children.push(new WhileExpr(form,
