  ...
  static from(form, analyzer) {
    ...
    return analyzer.with_locals(argv, ()=>{
      ...
      for (const [lhs, rhs] of bindings) {
        flat_bindings.push(...flatten_assignment_pair(lhs, rhs))
      }
...
class VarAssignment extends ASTNode {
  ...
  static from(form, var_sym, rhs_form, analyzer) {
    ...
    return new this(form, var_sym, rhs_form, meta_val)
  }
...
class DefExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    //   return new this(form, VarAssignment.from(form, bindings[0][0], bindings[0][1], analyzer))
    // }
    // return analyzer.with_locals_no_gensym(map(first, filter((pair)=>get(meta(pair[0]), "carrier"), bindings)), ()=> {
    //   return new this(form, new DoExpr(form, bindings.map((pair)=>{
    //     if (get(meta(pair[0]), "carrier")) {
    //       return new MutableAssignment(pair, JsIdentifierExpr.from(pair[0]), analyzer.analyze(pair[1]))
    //     }
    //     return VarAssignment.from(pair, pair[0], pair[1], analyzer)
    //   })))
    // })
  }
 ...
 */
function flatten_assignment_pair(lhs, rhs) {
  ...
  if (vector_p(lhs)) {
    // let res = []
    // let [new_lhs, as_sym] = VectorBinding.find_as(lhs)
    // if (as_sym) {
    //   // as-sym = <rhs-form>
    //   res.push([as_sym, rhs])
    //   rhs = as_sym
    // }
    // let more = []
    // new_lhs = new_lhs.map((o) => {
    //   if (is_sym_name(o, "&")) {
    //     return JsIdentifierExpr.from("...")
    //   }
    //   if (symbol_p(o)) {
    //     return o
    //   }
    //   const arg = gensym("a")
    //   more.push(...flatten_assignment_pair(o, arg, analyzer))
    //   return arg
    // })
    // res.push([new_lhs, list(as_iterator_qsym, rhs)],
    //          ...more)
    // return res
    // // Some special cases to keep emitted code simple
    // if (count(new_lhs) === 0) return []
    // if (count(new_lhs) === 1) return [[first(new_lhs), list(first_qsym, rhs)]]
    ...
    let carrier = with_meta(symbol("carrier"), dict("carrier", true)) // mark the carrier so we don't introduce a var
  ...
  if (dict_p(lhs)) {
    ...
    for (const [k, v] of lhs) {
      if (k instanceof Keyword) {
        ...
        res.push(...flatten_assignment_pair(k, list(get_qsym, as_sym, v)))
      }
    }
    return res
  }
...
class LetExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    return new this(form, [BindingScope.from(form, bindings, (a)=>a.analyze_forms(body), ConstAssignment, analyzer)])
  }
}
...
class LoopExpr extends ASTNode {
    ...
    tail_analyzer.analyze_forms =
      (forms)=>{
        if (forms.length === 0) {
          ...
          const [tail_recurs, tail] = LoopExpr.analyze_tail_form(last(forms), analyzer)
          tail_analyzer.tail_recurs = tail_recurs
          return analyzer.analyze_forms([tail])
        }
        const [tail_recurs, tail] = LoopExpr.analyze_tail_form(last(forms), analyzer)
        tail_analyzer.tail_recurs = tail_recurs
        return analyzer
    ...
    if (seq_p(form)) {
      ...
      if (initial instanceof Sym && initial.mod == null) {
        ...
        case "if":
          ...
          const [c_recurs, c] = this.analyze_tail_form(consequent, analyzer)
          const [a_recurs, a] = this.analyze_tail_form(alt, analyzer)
          return [c_recurs || a_recurs,
                  new IfStmt(form, analyzer.analyze(test), c, a)]
        case "recur":
          return [true, RecurExpr.from(form, analyzer)]
        case "do":
          const ta1 = this.make_tail_analyzer(analyzer)
          const do_expr = DoExpr.from(form, ta1)
          return [ta1.tail_recurs, do_expr]
        case "let":
          const ta2 = this.make_tail_analyzer(analyzer)
          const let_expr = LetExpr.from(form, ta2)
          return [ta2.tail_recurs, let_expr]
        default:
        }
      }
    }
    return [false, new ReturnStmt(form, analyzer.analyze(form))]
  }
  ...
  static analyze_body(form, body, analyzer) {
    if (body.length === 0) {
      ...
      const [tail_recurs, tail] = this.analyze_tail_form(last(body), analyzer)
      if (tail_recurs)
        return [new WhileExpr(form, new ConstantExpr(true), [tail])]
      return [tail]
    }
    const [tail_recurs, tail] = this.analyze_tail_form(last(body), analyzer)
    const body_analyzed = analyzer.analyze_forms(butlast(body)).concat([tail])
    if (tail_recurs)
      return [new WhileExpr(form, new ConstantExpr(true), body_analyzed)]
...
class RecurExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    return analyzer.with_loop_tail((binding_forms)=>{
      ...
      const bindings = reduce((acc,[lhs,rhs])=>acc.concat(flatten_assignment_pair(lhs, rhs)), [], map(list, binding_forms, rhss))
      return new this(
        form,
        [...bindings.map((pair)=>Reassignment.from(pair, pair[0], analyzer.analyze(pair[1]), analyzer)),
         ContinueStmt]
      )
    })
  }
