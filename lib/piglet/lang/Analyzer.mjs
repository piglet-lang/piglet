...
class FnTail extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    const bindings = flatten_assignment_pair(args_form, symbol("js:arguments"), analyzer)
    return new this(form, [], [BindingScope.from(form, bindings, rest, ConstAssignment, analyzer)], arity, varargs)
...
class BindingScope extends ASTNode {
  ...
  static from(form, [[lhs, rhs], ...bindings], body, assignment_type, analyzer) {
    const rhs_analyzed = analyzer.analyze(rhs)
    return analyzer.with_locals([lhs], ()=>{
      return new this(
        form,
        new assignment_type(
          form,
          analyzer.analyze(lhs),
          rhs_analyzed
        ),
        seq(bindings) ?
          [this.from(form, bindings, body, assignment_type, analyzer)] :
          analyzer.analyze_forms(body)
      )
    })
  }
}
...
class BindingPair extends ASTNode {
  static binding_type(lhs) {
    return (symbol_p(lhs) ? SymbolBinding :
  ...
  static from(pair, analyzer, assignment_type) {
    ...
    const pair_type = this.binding_type(lhs)
    if (!pair_type)
...
class VectorBinding extends BindingPair {
  ...
  static from(pair, lhs_orig, rhs, analyzer, Assignment) {
    ...
    if (as_sym) {
      ...
      self.children = [MutableAssignment.from(pair, analyzer, carrier_sym, rhs)]
    }
...
class DefExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    const bindings = flatten_assignment_pair(doc ? vary_meta(binding, assoc, doc_kw, doc) : binding,
                                             first(body),
                                             analyzer)
    if (bindings.length === 1) {
      return new this(form, VarAssignment.from(form, analyzer, bindings[0][0], bindings[0][1]), analyzer)
    }
    return new this(form, new DoExpr(form, bindings.map((pair)=>{
      if (get(meta(pair[0]), "carrier")) {
        return ConstAssignment.from(pair, analyzer, pair[0], pair[1])
      }
      return VarAssignment.from(pair, analyzer, pair[0], pair[1])
    })))
  }
 ...
 */
function flatten_assignment_pair(lhs, rhs, analyzer) {
  ...
  if (dict_p(lhs)) {
    ...
    for (const [k, v] of lhs) {
      if (k instanceof Keyword) {
        ...
        res.push(...flatten_assignment_pair(k, list(get_qsym, as_sym, v), analyzer))
      }
    }
    return res
  }
...
class LetExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    bindings = reduce((acc,[lhs,rhs])=>acc.concat(flatten_assignment_pair(lhs, rhs, analyzer)), [], partition_n(2, bindings))
    return new this(form, [BindingScope.from(form, bindings, body, ConstAssignment, analyzer)])
  }
}
...
class RecurExpr extends ASTNode {
  ...
  static from(form, analyzer) {
    ...
    console.log(binding_syms)
    const self = new this(form)
    self.children = Array.from(map(([lhs, lhs_gensym], rhs)=>BindingPair.from([lhs, rhs], analyzer, Reassignment),
                                   binding_syms,
                                   rest(form)))
...
export default class Analyzer {
  ...
  with_loop_bindings(bindings, thunk) {
    ...
    this.loop_bindings = bindings.map((s)=>[s, this.get_local(s)]) // pairs of [Sym(lhs), 'lhs123']
    thunk()
