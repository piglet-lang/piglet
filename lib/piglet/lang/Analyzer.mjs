...
import {partition_n, munge} from "./util.mjs"
...
function set_current_module(v) {
    return resolve(symbol("piglet", "lang", "*current-module*")).set_value(v)
}
...
class ASTNode {
    ...
    emit(cg) {
        if (this.children) {
            return this.children.map((c)=>cg.emit(this, c))
        }
...
class FnExpr extends ASTNode {
    ...
    emit(cg) {
        return cg.function_expr(this, {name: this.name ? cg.emit(this.name, munge(this.name.name)) : null,
...
class ModuleExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        const parsed_mod = Module.from(compiler().pkg, form)
        const interned_mod = ensure_module(parsed_mod.pkg, parsed_mod.name)
        Object.assign(interned_mod.imports, parsed_mod.imports)
        Object.assign(interned_mod.aliases, parsed_mod.aliases)
        interned_mod.self_ref = symbol(`mod-${interned_mod.pkg}-${interned_mod.name}`)
        analyzer.push_locals([interned_mod.self_ref])
        const analyze = analyzer.analyze.bind(analyzer)
        const self = analyze(interned_mod.self_ref)
        const children = [
            (new VariableDeclaration(
                this,
                self,
                analyze(
                    list(symbol('piglet:lang:ensure-module'), interned_mod.pkg, interned_mod.name)
                ))),
            analyze(list(symbol('piglet:lang:intern'),
                         list(symbol('quote'), symbol('piglet:lang:*current-module*')),
                                  self)),
            ...(interned_mod.imports.map(({from, alias})=>{
                if (from instanceof PrefixName) {
                    return analyze(list(symbol('piglet:lang:require'), list(symbol('piglet:lang:prefix-name'), from.prefix, from.suffix)))
                } else if (typeof from === 'string') {
                    return analyze(list(symbol('.set_alias'), self, alias.name, list(symbol('await'), list(symbol('piglet:lang:js-import'), from))))
                }
            }))
        ]
    ...
    // [
        //     cg.invoke_var(this.form, 'piglet', 'lang', 'intern', [
        //         symbol('piglet:lang:*current-module*').emit(cg),
        //         cg.invoke_var(this, 'piglet', 'lang', 'ensure_module', [
        //             this.module.pkg, this.module.name
        //         ])
        //     ].concat(this.module.imports.map(({from, alias})=>{
        //         if (from instanceof PrefixName) {
        //             return cg.await_expr(
        //                 this,
        //                 cg.invoke_var(this.form, 'piglet', 'lang', 'require', [from])
        //             )
        //         } else if (typeof from === 'string') {
        //             return cg.define_var(this, )
        //             cg.await_expr(
        ...
        if (seq_p(form)) {
            ...
            if (initial instanceof Sym) {
                ...
                if (get(meta(the_var), keyword("macro"))) {
                    const expanded = the_var.invoke(...rest(form))
