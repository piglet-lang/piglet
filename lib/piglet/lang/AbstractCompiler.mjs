...
export default class AbstractCompiler {
  ...
  async load(mod) {
    ...
    const current_pkg = resolve(symbol('piglet:lang:*current-package*')).value.name
    const slurped_mod = await this.slurp_mod(current_pkg, this.mod_name)
    if (!slurped_mod)
      throw new Error(`Failed to load module ${this.mod_name} from ${current_pkg}`)
    const [source, location] = slurped_mod
    ...
    if (this.aot_compiling) {
      ...
      if (mod.fqn.toString() === "https://piglet-lang.org/packages/piglet:lang") {
        js = `${await this.slurp(new URL("../lang.mjs", import.meta.url))}\n${js}`
      }
  ...
  async register_package(location_href, pkg_spec) {
    // console.log("register_package", {location_href, pkg_spec})
  ...
  async eval(form) {
    ...
    const result = await eval?.(js) // indirect eval
    if (this.aot_compiling) {
