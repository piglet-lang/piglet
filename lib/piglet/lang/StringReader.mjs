...
export default class StringReader {
    ...
    _read() {
        ...
        if (this.eof()) {
            ...
            if (new_meta instanceof Sym) {
                ...
                next_meta = Array.from(meta).reduce((acc, [k, v])=>Associative._assoc(acc, k, v), next_meta)
            }
    ...
    read() {
        ...
        if (expr && typeof expr === 'object') {
            ...
            if (WithMeta.satisfied(expr)) {
                ...
                if (has_meta(expr)) {
                    console.log("Meta already set!", expr, meta(expr))
                } else {
                    set_meta(expr, m)
                }
            }
    ...
    read_string() {
        ...
        return eval(str)//new Primitive(str, eval(str))
    }
