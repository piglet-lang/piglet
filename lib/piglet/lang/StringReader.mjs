...
export default class StringReader {
    ...
    read_number() {
        ...
        if (ch_dash === this.cc)
            this.next_ch()
        if (ch_0 === this.cc && this.next_ch(), ch_x === this.cc) {
            this.next_ch()
            return this.read_hex(this.input.charCodeAt(start) === ch_dash)
        }
    ...
    at_hex_char() {
        return (ch_0 <= this.cc && this.cc <= ch_9) ||
            (ch_a <= this.cc && this.cc <= ch_f) ||
            (ch_A <= this.cc && this.cc <= ch_A)
    }

    read_hex(negative) {
        const start = this.pos
        while (!this.eof() && this.at_hex_char()) this.next_ch()
        const num_str = this.input.substring(start, this.pos)
        return parseInt(num_str, 16)
    }
