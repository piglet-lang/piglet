...
export default class StringReader {
    ...
    if (this.eof()) {
      ...
      if (this.cc === ch_underscore) {
        ...
        while (!this.eof() && ch_dubquot != this.cc) {
          if (ch_backslash === this.cc) this.next_ch(true)
          this.next_ch(true)
        }
        this.skip_char(ch_dubquot)
        return new RegExp(this.input.substring(start, this.pos-1), undefined)
      } else if(this.cc === ch_bang) { // skip shebang
        while (!this.eof() && ch_eol != this.cc) this.next_ch(true)
        return this._read()
      } else {
  ...
  read_with_meta() {
    ...
    if (Array.isArray(expr)) {
      return set_meta(expr, this.meta_stack.shift())
    }
  ...
  read_percent_form() {
    ...
    const type = this.ch
    if (!((ch_a <= this.cc && this.cc <= ch_z) ||
          (ch_A <= this.cc && this.cc <= ch_Z))) {
      if (this.reading_anon_fn) {
        this.anon_argcnt = Math.max(this.anon_argcnt, 1)
        return Sym.parse("%1")
      }
      return Sym.parse("%")
    } else {
