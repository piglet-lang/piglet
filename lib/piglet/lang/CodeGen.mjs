...
import {assert, munge} from "./util.mjs"
...
export default class CodeGen {
    ...
    oget(node, o, k) {
        return mknode('MemberExpression', {node: node, object: o, property: k, computed: true})
    }

    var_value(node, sym) {
        return this.oget(node, this.var_ref(node, sym), this.literal(node, "value"))
    }

    module_ref(node, pkg, mod) {
        return this.member_lookup(node,
                                  this.oget(node,
                                            this.identifier(node, "$piglet$"),
                                            this.literal(node, pkg)),
                                  [mksym(node, "modules"),
                                   mksym(node, munge(mod)),
                                  ])
    }
