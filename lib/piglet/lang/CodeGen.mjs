...
function mknode(type, {node, end_node, ...props}) {
    ...
    const start = Lookup._get(meta(node), kw_start)
    const end = Lookup._get(meta(node), kw_end)
    const line = Lookup._get(meta(node), kw_line)
    const col = Lookup._get(meta(node), kw_col)
    if (start) {
        out.start = start
