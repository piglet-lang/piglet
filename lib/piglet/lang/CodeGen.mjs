...
export default class CodeGen {
    ...
    function_call(node, callee, args) {
        let n = this.mknode('CallExpression',
                            {node: node,
                             callee: this.mknode('MemberExpression', {node: node,
                                                                     object: callee,
    ...
    var_reference(node, sym) {
        ...
        return this.member_lookup(node,
                                  ...
                                  [this.mksym(node, Module.munge(the_var.pkg)),
                                   this.mksym(node, "modules"),
                                   this.mksym(node, Module.munge(the_var.module)),
                                   this.mksym(node, "vars"),
                                   this.mksym(node, Module.munge(sym.name)),
                                   this.mksym(node, "value")])
    ...
    define_var(node, name, value, meta) {
        return this.method_call(node,
                                "intern",
                                this.member_lookup(node, this.identifier(node, "$piglet$"),
                                                   [this.mksym(node, Module.munge(this.current_module().pkg)),
                                                    this.mksym(node, "modules"),
                                                    this.mksym(node, Module.munge(this.current_module().name))]),
    ...
    invoke_var(node, pkg, mod, name, args) {
        return this.method_call(
            ...
            this.method_call(
                ...
                "resolve",
                this.member_lookup(node, this.identifier(node, "$piglet$"),
                                   [this.mksym(node, Module.munge(pkg)),
                                    this.mksym(node, "modules"),
                                    this.mksym(node, Module.munge(mod))]),
    ...
    new_expr(node, ctor, args) {
        return this.mknode('NewExpression', {node: node, callee: ctor, arguments: args})
    }

    rest_element(node, arg) {
        return this.mknode('RestElement', {node:node, argument: arg})
    }

    block_expr(node, body) {
        return this.mknode('BlockExpression', {node:node, body: body})
    }

    block_stmt(node, body) {
        return this.mknode('BlockStatement', {node:node, body: body})
    }

    expr_stmt(node, expression) {
        return this.mknode('ExpressionStatement', {node:node, expression: expression})
    }

    assignment(node, left, right) {
        return this.mknode('AssignmentExpression',
                           {node:node,
                            operator: '=',
                            left: left,
                            right: right})
