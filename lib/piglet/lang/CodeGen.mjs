...
import {resolve, symbol, module_registry} from "../lang.mjs"
...
export default class CodeGen {
    ...
    as_expression(node) {
        if (Array.isArray(node)) {
            if (node.length == 0) {
                return mknode('Literal', {value: null, raw: 'null'})
            }
            if (node.length == 1) {
        ...
        return this.wrap_iife(node, [node])
    }
    ...
    function_expr(node, {name, argv, body, async_p}) {
    ...
    var_reference(node, sym) {
        ...
        if (!the_var) {
            console.log(module_registry.inspect())
    ...
    function_call(node, callee, args) {
        return mknode('CallExpression',
                      {node: node,
                       callee: this.as_expression(callee),
                       arguments: args.map((a)=>this.as_expression(a))})

        // This version always uses `.call`, so that it works with function-like things that aren't actually functions.
        // The problem is it changes the value of "this" in interop calls.
        // return mknode('CallExpression',
        //               {node: node,
        //                callee: mknode('MemberExpression', {node: node,
        //                                                    object: this.as_expression(callee),
        //                                                    property: this.identifier(node, "call"),
        //                                                    computed: false}),
        //                arguments: [this.literal(node, null), ...args.map((a)=>this.as_expression(a))]})
    }
    ...
    wrap_iife(node, body) {
        return mknode(
            ...
            {node: node,
             arguments: [],
             callee: this.function_expr(node, {name: null,
                                               argv: [],
                                               body: body,
                                               async_p: false})})
    }
    ...
    wrap_async_iife(node, body) {
        return mknode(
            ...
            {node: node,
             callee:
             this.function_expr(node, {name: null,
                                       argv: [],
                                       body: body,
                                       async_p: true})})
    }
