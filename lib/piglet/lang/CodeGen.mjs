...
function mknode(type, {node, end_node, ...props}) {
    ...
    const m = meta(node)
    if (m) {
        out.start = Lookup._get(m, kw_start)
        out.end = Lookup._get(m, kw_end)
        out.line = Lookup._get(m, kw_line)
        out.col = Lookup._get(m, kw_col)
    }
    if (out.start) {
...
export default class CodeGen {
    ...
    emit(node, value) {
        ...
        if(Array.isArray(value)) {
            ...
            if (meta(value)) {
                console.log("ARRAY META", meta(value))
            }
