...
export default class CodeGen {
    ...
    literal(node, value, raw) {
        return mknode('Literal', {node: node, value: value, raw: (raw === undefined && value === undefined ? "undefined" : JSON.stringify(value))})
    }
    ...
    while_stmt(node, test, body) {
        return mknode('WhileStatement',
                      {node: node,
                       test: this.emit(test, test),
                       body: this.as_statement(this.emit(body, body))})
    }
