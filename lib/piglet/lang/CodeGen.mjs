...
import {assert, munge, partition_n_step, PIGLET_PKG, valid_property_name_p} from "./util.mjs"
import Lookup from "./protocols/Lookup.mjs"

function expression_p(node) {
  return node.type.endsWith('Expression') ||
    node.type.endsWith('Element') ||
    node.type === 'Literal' ||
...
export default class CodeGen {
  ...
  function_expr(node, {name, argv, body, async_p, meta}) {
    // METAHACKS: disabling this for now, as it invokes quite a performance
    // penalty. Metadata in general needs to be revisited. Functions should be
    // able to carry metadata, but not every function needs full location
    // metadata (we can reserve that for vars), and which metadata keys are
    // preserved elided needs to be configurable based on the compilation
    // profile. (e.g. elide location metadata when AOT compiling)
    // See also other METAHACKS notes.

    const fn_expr = mknode('FunctionExpression',
                           {node: node,
                            id: name,
                            expression: false,
                            generator: false,
  ...
  oget(node, o, k) {
    if ('Literal' === k.type && valid_property_name_p(k.value)) {
      return mknode('MemberExpression', {node: node, object: this.as_expression(o), property: this.identifier(k, k.value), computed: false})
    }
  ...
  var_ref(node, sym) {
    ...
    return this.member_lookup(node,
                              this.oget(node,
                                        ...
                                        // the_var.pkg === PIGLET_PKG ?
                                        // this.literal(node, "piglet", "piglet") :
  ...
  method_call(node, method, object, args) {
    return this.call_expr(
      ...
      mknode('MemberExpression',
             {node: node,
              object: this.as_expression(object),
  ...
  rest_element(node, arg) {
    return mknode('RestElement', {node:node, argument: mknode('Identifier', {node:node, name: arg})})
  }
