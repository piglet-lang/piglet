...
export function hash_code(o) {
  ...
  if ("object" === t || "function" === t) {
    ...
    // ^ taking this out because non-frozen objects get a fixed incrementing
    // integer id, relying on the fact that this gets cached and so is stable,
    // otherwise each called to `hash` yields a new version, and so objects are
    // not equal to themselves. This does mean that mutable objects should not
    // have a value-based hash implementation, which also means that standard
    // piglet equality semantics don't apply to mutable containers, which may or
    // may not be what we want... needs revisiting
