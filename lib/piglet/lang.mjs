...
export function qname(name, separator, suffix) {return new QName(null, name, separator, suffix)}
self.intern("qname", qname)
...
export function reduce(rf, ...args) {
    ...
    if (async_p) {
        ...
        if (seq(coll)) next(acc)
        acc = acc.then((acc)=>{if (seq(coll)) { return next(acc)}; return acc;})
    } else {
        while (seq(coll)) {
            acc = rf(acc, first(coll))
            coll = rest(coll)
...
export async function js_import(path) {
    const mod = module_registry.resolve_module(deref(self.resolve("*current-package*")).name, path)
    if (mod.required) {
        return mod
    }
