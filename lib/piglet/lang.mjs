...
self.intern('*console*', console, dynamic_meta)
self.intern('piglet-base-url', new URL("../..", import.meta.url))
const _$STAR$_print_meta_$STAR$_ = self.intern('*print-meta*', false)
...
;[
  ...
  proto.intern(self)
  reset_meta(self.resolve(proto.name), link_direct_meta)
  mark_as_piglet_object(proto)
})

export {AbstractIdentifier, AbstractSeq, Cons, Context, Dict, IteratorSeq, Keyword,
        LazySeq, List, Module, Package, PrefixName, Protocol, QName, QSym, Range,
        Repeat, SeqIterator, Sym, Var, HashSet, MutableCollection}
...
self.intern("list", list, link_direct_meta)

export function list_STAR(...args) { return list(...butlast(args), ...last(args)) }
self.intern("list*", list_STAR, link_direct_meta)
...
self.intern("range", range, link_direct_meta)

export function symbol_p(s) {
  return s instanceof Sym
}
self.intern("symbol?", symbol_p), link_direct_meta
...
self.intern("keyword", keyword, link_direct_meta)
self.intern("symbol", symbol, link_direct_meta)
self.intern("set", set), link_direct_meta

export function keyword_p(o) {return o instanceof Keyword}
self.intern("keyword?", keyword_p), link_direct_meta
...
self.intern("qname", qname, link_direct_meta)

export function qsym(name) {return new QSym(null, name)}
self.intern("qsym", qsym, link_direct_meta)
...
self.intern("prefix-name", prefix_name, link_direct_meta)

export function prefix_name_p(o) {return o instanceof PrefixName}
self.intern("prefix-name?", prefix_name_p), link_direct_meta

export function qname_p(o) {return o instanceof QName}
self.intern("qname?", qname_p), link_direct_meta

export function qsym_p(o) {return o instanceof QSym}
self.intern("qsym?", qsym_p), link_direct_meta
...
self.intern("name", name, link_direct_meta)

export function mod_name(sym) {
  return sym.mod
}
self.intern("mod-name", mod_name, link_direct_meta)

export function pkg_name(sym) {
  return sym.pkg
}
self.intern("pkg-name", pkg_name, link_direct_meta)
...
self.intern("type", type, link_direct_meta)

export function fqn(o) {
  return QualifiedName._fqn(o)
}
self.intern("fqn", fqn, link_direct_meta)
...
self.intern("find-package", find_package, link_direct_meta)
...
self.intern("find-module", find_module, link_direct_meta)
...
self.intern("ensure-module", ensure_module, link_direct_meta)
...
self.intern("resolve", resolve, link_direct_meta)
...
self.intern("inspect", inspect, link_direct_meta)
...
self.intern("intern", intern, link_direct_meta)
...
self.intern("conj", conj, link_direct_meta)
...
self.intern("conj!", conj_BANG, link_direct_meta)

export function cons(val, s) {
  return new Cons(val, Seq.satisfied(s) ? s : seq(s))
}
self.intern("cons", cons, link_direct_meta)

export function satisfies_p(protocol, obj) {
  return protocol.satisfied(obj)
}
self.intern("satisfies?", satisfies_p, link_direct_meta)

export {munge, unmunge, partition_n}
self.intern("munge", munge, link_direct_meta)
self.intern("unmunge", unmunge, link_direct_meta)
self.intern("partition", partition_n_step, link_direct_meta)
self.intern("partition-all", partition_all_n_step, link_direct_meta)
...
self.intern("fn?", fn_p, link_direct_meta)

export function array_p(o) {
  return Array.isArray(o)
}
self.intern("array?", array_p, link_direct_meta)

export function iterator_p(o) {
  return o && fn_p(o.next)
}
self.intern("iterator?", iterator_p, link_direct_meta)

export function iterable_p(o) {
  return !!o?.[Symbol.iterator]
}
self.intern("iterable?", iterable_p, link_direct_meta)
...
export function seq(o) {
  ...
  if (iterable_p(o)) {
    return IteratorSeq.of_iterable(o)
  }
  if (seqable_p(o)) {
    return Seqable._seq(o)
  }
  if (typeof o === "object") {
    return IteratorSeq.of_iterable(Object.entries(o))
  }
  throw new Error(`${inspect(o)} ${typeof o} is not seqable`)
}
self.intern("seq", seq, link_direct_meta)

export function first(o) {
  return Seq._first(seq(o))
}
self.intern("first", first, link_direct_meta)

export function second(o) {
  return first(rest(o))
}
self.intern("second", second, link_direct_meta)

export function third(o) {
  return first(rest(rest(o)))
}
self.intern("third", third, link_direct_meta)

export function fourth(o) {
  return first(rest(rest(rest(o))))
}
self.intern("fourth", fourth, link_direct_meta)
...
self.intern("nth", nth, link_direct_meta)
...
self.intern("rest", rest, link_direct_meta)

export function make_lazy_seq(thunk) {
  return new LazySeq(thunk)
}
self.intern("make-lazy-seq", make_lazy_seq, link_direct_meta)
...
self.intern("array-str", array_str, link_direct_meta)

function object_string_tag(o) {
  return o[Symbol.toStringTag]
}
self.intern("object-string-tag", object_string_tag, link_direct_meta)
...
export function print_str(o, ...args) {
  ...
  try {
    ...
    try {
      ...
      if (typeof o === 'object') {
        let s = ""
        if (meta(o) && _$STAR$_print_meta_$STAR$_.deref()) {
          s+=`^${print_str(meta(o))} `
        }
...
self.intern("print-str", print_str, link_direct_meta)

export function println(...args) {
  deref(self.resolve("*console*")).log(...Array.from(args, (a)=>(typeof a === 'string') ? a : print_str(a)))
  return null
}
self.intern("println", println, link_direct_meta)

export function prn(...args) {
  deref(self.resolve("*console*")).log(...Array.from(args, (a)=>print_str(a)))
  return null
}
self.intern("prn", prn, link_direct_meta)
...
self.intern("Reduced", Reduced, link_direct_meta)
...
self.intern("reduced?", reduced_p, link_direct_meta)

export function async_fn_p(f) {
  return typeof f === 'function' && f.constructor.name === 'AsyncFunction'
}
self.intern("async-fn?", async_fn_p, link_direct_meta)

export function reduce(rf, ...args) {
  let coll, acc
  const async_p = async_fn_p(rf)
  if (args.length == 2) {
    acc = args[0]
    coll = args[1]
  } else {
  ...
  if (async_p) {
    async function next(acc) {
      acc = await rf(acc, first(coll))
      if (reduced_p(acc)) return acc.value
      coll = rest(coll)
      if (seq(coll)) return await next(acc)
      return acc
    }
    return next(acc)
  } else {
...
self.intern("reduce", reduce, link_direct_meta)
  ...
  if (arguments.length === 1) {
    return function(rf) {
      return function(result, input) {
        ...
        case 0: return rf()
        case 1: return rf(result)
        case 2: return rf(result, f(input))
        }
      }
    }
  }
...
self.intern("map", map, link_direct_meta)
  ...
  if (arguments.length === 1) {
    return function(rf) {
      return function(result, input) {
        ...
        case 0: return rf()
        case 1: return rf(result)
        case 2: return pred(input) ? rf(result, input) : result
        }
      }
    }
  }
  ...
  if (seq(coll)) {
    const res = (s)=> {
      if (seq(s)) {
        const el = first(s)
        if (pred(el)) {
          return cons(el, make_lazy_seq(()=>res(rest(s))))
        }
        return make_lazy_seq(()=>res(rest(s)))
      }
    }
    return res(coll)
  }
}
self.intern("filter", filter, link_direct_meta)
...
self.intern("remove", remove, link_direct_meta)

export function reverse(coll) {
  return reduce((acc,e)=>cons(e,acc), null, coll)
}
self.intern("reverse", reverse, link_direct_meta)

export function complement(pred) {
  return (el)=>!pred(el)
}
self.intern("complement", complement, link_direct_meta)
...
self.intern("-", minus, link_direct_meta)
...
self.intern("*", multiply, link_direct_meta)
...
self.intern("assoc", assoc, link_direct_meta)
...
self.intern("assoc!", assoc_$BANG$_, link_direct_meta)
...
self.intern("dissoc", dissoc, link_direct_meta)

export function dissoc_$BANG$_(m, k) {
  return MutableAssociative._dissoc_$BANG$_(m, k)
}
self.intern("dissoc!", dissoc_$BANG$_, link_direct_meta)
...
self.intern("get", get, link_direct_meta)
...
self.intern("keys", keys, link_direct_meta)
...
self.intern("vals", vals, link_direct_meta)
...
self.intern("alter-meta!", alter_meta, link_direct_meta)
...
self.intern("count", count, link_direct_meta)
...
export const _$EQ$_ = eq

export function truthy_p(v) {
  return v !== false && v !== null && v !== undefined
}
self.intern("truthy?", truthy_p, dict(keyword("tag"), symbol("boolean"), keyword("link-direct"), true))

export function falsy_p(v) {
  return v === false || v === null || v === undefined
}
self.intern("falsy?", falsy_p, dict(keyword("tag"), symbol("boolean"), keyword("link-direct"), true))
...
self.intern("and", and, link_direct_meta)
...
self.intern("or", or, link_direct_meta)

export function not(v) {
  return falsy_p(v) ? true : false
}
self.intern("not", not, link_direct_meta)

export {gensym}
self.intern("gensym", gensym, link_direct_meta)

export function _import(mod) {
...
self.intern("import", _import, link_direct_meta)
self.intern("require", _import, link_direct_meta) // deprecate
export const require = _import
...
export async function js_import(mod_path) {
  ...
  for(const [k, v] of Object.entries(imported)) {
    mod.intern(unmunge(k), v, link_direct_meta)
  }
  mod.is_js_import = true
  mod.required = true
  return mod
}
self.intern("js-import", js_import, link_direct_meta)
...
self.intern("apply", apply, link_direct_meta)

export function butlast(arg) {
  if (nil_p(arg)) return list()
  return Array.from(arg).slice(0, -1)
}
self.intern("butlast", butlast, link_direct_meta)
...
self.intern("last", last, link_direct_meta)
...
self.intern("select-keys", select_keys, link_direct_meta)
...
Empty.extend2(
  ...
  Object, {"-empty?/1": (self) => {if (Object != self.constructor) return false; for (let _ in self) return false; return true}},
...
Eq.extend2(
  ...
  String, {"-eq/2": (self, other) => { return (typeof other === 'string' || other instanceof String) && self == other }},
  Boolean, {"-eq/2": (self, other) => { return self === other }},

  Sym, {"-eq/2": (self, other) => { return self.eq(other) }},

  Array, {"-eq/2": (self, other) => {
    // Both are arrays, we can rely on fast indexed access
    if (Array.isArray(other) && self.length == other.length) {
      return self.reduce((acc, val, idx)=>acc && eq(val, other[idx]), true)
    }
    // Short circuit based on collection count, if possible
    if (Counted.satisfied(other) && self.length !== Counted._count(other)) {
      return false
    }
    if (seq_p(other) || seqable_p(other)) {
      let v = first(other)
      other = rest(other)
      return self.reduce((acc, val, idx)=>{
        acc = acc && eq(val, v)
        if (acc) {
          v = first(other)
          other = rest(other)
        ...
        return acc
      }, true) && other === null // If "other" is not a counted collection, then make sure the reduce has fully consumed it
    }
    return false
  }},
...
Hashable.extend2(
  ...
  String, {"-hash-code/1": (self) => { return hash_str(self.toString()) }},
  Symbol, {"-hash-code/1": (()=>{
...
Repr.extend2(
  ...
  Array,  {"-repr/1": (self) => {return `${array_str(self)}`}},
...
Seqable.extend2(
  ...
  // If we do this, then every object becomes seqable, including e.g. numbers,
  // which is not very useful. Instead we handle "plain" objects directly inside [[seq]]
...
WithMeta.extend2(
  ...
  Array, {"-with-meta/2": (arr, metadata) => {
    if (!eq2(meta(arr), metadata))
      return set_meta(Array.from(arr), metadata)
    return arr
  }},
)
