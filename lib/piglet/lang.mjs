...
export function dissoc_$BANG$_(m, k) {
    const res = MutableAssociative._dissoc_$BANG$_(m, k)
    if (meta(m)) set_meta(res, meta(m))
    return res
}
...
self.intern("eval-verbose", async function (form, verbosity) {
    if (!self.resolve("*compiler*").deref()) {
        throw new Error("No compiler present, can't eval.")
    }
    return await self.resolve("*compiler*").deref().eval_verbose(form, verbosity)
})
...
Hashable.extend(
    ...
    BigInt, [(function _hash_code(self) { return hash_num(self) })],
    String, [(function _hash_code(self) { return hash_str(self) })],
    Symbol, [(function _hash_code(self) { return Math.floor(Math.random() * 100000000) })],
    RegExp, [(function _hash_code(self) { return hash_str(`/${self.toString()}/`) })],
    ...
    [(function _hash_code(self) { return self.map((e)=>Hashable._hash_code(e)).reduce(hash_combine, 1) })],

    Set,
    [(function _hash_code(self) { return self.map((e)=>Hashable._hash_code(e)).reduce(hash_combine, 2) })],
    ...
    AbstractSeq, [(function _hash_code(self) { return Array.from(self, (e)=>Hashable._hash_code(e)).reduce(hash_combine, 3) })],
    ...
    HashSet, [(function _hash_code(self) { return reduce((acc,o)=>hash_combine(acc, hash_code(o)), 5, self) })],
