...
import {PIGLET_PKG, munge, unmunge, partition_n} from "./lang/util.mjs"
...
export function qsym(name) {return new QSym(name)}
self.intern("qsym", qsym)
...
export function find_module(pkg, mod) {
    if (pkg instanceof PrefixName) {
        throw new Error("PrefixName is for data, not for code references: got ${pkg}:${mod}")
        // return find_module(pkg.prefix || self.resolve("*current-module*").deref().pkg, pkg.suffix)
    }
    if (pkg.includes("://")) {
        return module_registry.find_module(pkg, mod)
    } else {
        const current_module = self.resolve("*current-module*").deref()
        const expanded = current_module.package.aliases[pkg]
        if (!expanded) {
            throw new Error(`No alias ${pkg} found in ${current_module.package.name}`)
        }
        pkg = expanded
    }
    return module_registry.find_module(pkg, mod)
...
Repr.extend(
    ...
    QName, [function _repr(qname) {
        if (eq(Derefable.deref(self.resolve("*qname-print-style*")), keyword('compact')) ) {
            return self.resolve("*current-context*").deref().contract(qname).toString()
        } else {
