...
// Intern classes
self.intern("AbstractIdentifier", AbstractIdentifier)
...
self.intern("Module", Module)
self.intern("Named", Named)
self.intern("Package", Package)
self.intern("PrefixName", PrefixName)
self.intern("Protocol", Protocol)
...
TaggedValue.intern(self)
Walkable.intern(self)
WithMeta.intern(self)
QualifiedName.intern(self)
...
export function print_str(o) {
    ...
    if (typeof o === 'object') {
        ...
        if (o?.constructor) {
            ...
            if (tag_name !== "Object") {
                return s + "#js:" + (o[Symbol.toStringTag] || o.constructor?.name) + " {" +Object.getOwnPropertyNames(o).map(k=>":"+k+" "+print_str(o[k])).join(", ") + "}"
            }
        }
...
export function require(mod) {
    if (symbol_p(mod)) { // resolve sym to qsym
        mod = qsym(`${self.resolve("*current-package*").deref().name}:${mod}`)
    }
...
export async function js_import(mod_path) {
    ...
    if (compiler) {
        ...
        if (!path) {
            throw new Error(`Could not find JS module ${mod_path}`)
        }
    } else {
        path=mod_path
    }
...
Lookup.extend(
    ...
    [function _get(mod, k) { return Lookup._get(mod, k, null)},
     function _get(mod, k, v) { return mod.lookup(name(k)) || v }],
)
