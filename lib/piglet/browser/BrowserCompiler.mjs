...
export default class BrowserCompiler extends AbstractCompiler {
    ...
    async slurp_mod(pkg_name, mod_name) {
        ...
        for (const dir of pkg.paths) {
            console.log(`${mod_name}.pig`, dir, pkg.location, window.location)
            const loading_path = new URL(`${mod_name}.pig`, new URL(`${dir}/`, new URL(pkg.location, window.location))).href
            const response = await fetch(loading_path)
            if (response.ok) return [await response.text(), loading_path]
        }
}
