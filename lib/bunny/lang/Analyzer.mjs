...
import {list, symbol, resolve, rest, intern, remove} from "../lang.mjs"
...
class FnExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        argv = x
        analyzer.push_locals(Array.from(argv))
...
class MacroVarExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        current_module().intern(var_sym.name, null, Object.assign({macro: true}, var_sym.meta))
...
class DefExpr extends ASTNode {
    ...
    static from(form, analyzer) {
        ...
        current_module().intern(var_sym.name, null, var_sym.meta)
        ...
        if (form instanceof List) {
            ...
            if (initial instanceof Sym) {
                if (initial.name.endsWith(".")) {
                    return new NewInstanceExpr(form, this)
                }
