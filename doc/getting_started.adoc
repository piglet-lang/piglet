...
The easiest way to get started is the `bin/piglet` script. You can symlink this
from somewhere on your `PATH` to get a `piglet` command in your shell.

Without any arguments this drops you into a REPL, or can you can specify
expressions (-e), packages (-p), or modules (-i) to load and run.

Under the hood this just invokes node like this, but it does a couple of extra
